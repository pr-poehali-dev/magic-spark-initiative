    ...
    >
      <nav className="container mx-auto px-6 flex items-center justify-between md:px-[24]">
        <a href="/" className="flex items-center gap-2 group" onClick={scrollToTop}>
          <span className="text-white text-xl font-semibold tracking-tight">Roomiq</span>
        </a>

        <ul className="hidden md:flex items-center gap-10 text-sm tracking-wide">
          {[
            { label: "Главная", href: "#hero" },
            { label: "Возможности", href: "#about" },
            { label: "Примеры", href: "#projects" },
            { label: "Функции", href: "#services" },
        ...
        <button
          onClick={onSignUp}
          className={cn(
            "hidden md:inline-flex items-center gap-2 text-sm px-5 py-2.5 transition-all duration-300 cursor-pointer",
            scrolled
      ...
      >
        <div className="container mx-auto px-6">
          ...
          <button
            className="inline-flex items-center justify-center gap-2 text-sm px-5 py-2.5 bg-white text-foreground border border-foreground/20 hover:bg-foreground hover:text-white transition-all duration-300 mb-4 cursor-pointer"
